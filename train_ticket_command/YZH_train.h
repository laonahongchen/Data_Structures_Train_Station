...
struct train{
	...
	train(const mystring<20> &id, const mystring<40> &na, const char &cat, int num_sta, int num_pri, const mystring<40>* name_pri,
	...
	void sale_train(bplus_tree<char> &find_train, bplus_tree<char> &location) {
		...
		for (int i = 0; i < num_station - 1; ++i) {
			for (int j = i + 1; j < num_station; ++j) {
				mystring<100> Comkey = sta[i].name + sta[j].name + train_id;
	...
	bool modify(const mystring<40> &na, const char &cat, int num_sta, int num_pri, const mystring<40>* name_pri,
		const train_station *s)
	{
		if (besaled) {
			cout << 0 << '\n';
	...
	bool buy(int num, const mystring<40> &loc1, const mystring<40> &loc2, int date, const mystring<40> &kind) {
		...
		for (ticket_kind = 0; ticket_kind < num_price; ++ticket_kind) {
			if ((mystring<40>)kind == name_price[ticket_kind])
				break;
		}
		...
		for (loc1_pos = 0; loc1_pos < num_station - 1; ++loc1_pos) {
			if ((mystring<40>)loc1 == sta[loc1_pos].name)
				break;
		}
		...
		for (loc2_pos = loc1_pos + 1; loc2_pos < num_station; ++loc2_pos) {
			if ((mystring<40>)loc2 == sta[loc2_pos].name)
				break;
		}
	...
	bool refund(int num, const mystring<40> &loc1, const mystring<40> &loc2, int date, const mystring<40> &kind) {
		int ticket_kind, loc1_pos, loc2_pos;
	...
	void queryTicket(const mystring<40> &loc1, const mystring<40> &loc2, int date){
...
bool buy_ticket(int num, const mystring<20> &trainid, const mystring<40> &loc1, const mystring<40> &loc2, 
	int date, const mystring<40> &kind, bplus_tree<train> &thetrain) 
{
	train pos;
	...
	if (pos.buy(num, loc1, loc2, date, kind)) {
		cout << 1 << '\n';
...
bool refund_ticket(int num, const mystring<20> &trainid, const mystring<40> &loc1, const mystring<40> &loc2,
	int date, const mystring<40> &kind, bplus_tree<train> &thetrain)
{
	train pos;
	if (thetrain.search(trainid, &pos) != 0) {
